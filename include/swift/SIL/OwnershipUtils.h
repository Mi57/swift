...
/// scope. The "use points" are the relevant points for determining
/// lifetime. The use points discovered by this function are all dominated by
/// the guaranteed value and may include reborrows.
///
...
///
/// Return true if all uses were analyzed, a visitor callback did not exit
/// early, and no PointerEscape uses were discovered. PointerEscapes are
/// allowed, however, if they are within a borrow scope either introduced by
/// this guaranteed value or a nested borrow--these escapes must still be
/// contained within the scope, so they are irrelevant to lifetime.
///
/// Nested borrow scopes are considered uses at the point where their "extended"
/// borrow scope ends. An extended borrow scope is found by looking through any
/// reborrows that end the nested scope. Other uses within those extended borrow
/// scopes are ignored.
///
/// If this guaranteed value introduces a borrow scope (begin_borrow,
/// load_borrow, or phi), then its only use points are the scope-ending uses
/// (via BorrowedValue::visitLocalScopeEndingUses). This is similar to handling
/// a nested borrow scope, but in this case reborrows are considered scope
/// ending uses and recorded as use points. (Guaranteed values that do not
/// introduce a borrow scope never have reborrow use points).  The \p
/// visitReborrow callback can be used to transitively process reborrows to
/// discover the extended lifetime. Reborrows may be recursive, so this will
/// require checking membership in a working set. Note that \p visitReborrow is
/// not called for nested reborrows because reborrows within nested borrow scope
/// are irrelevant to lifetime.
bool findTransitiveGuaranteedUses(SILValue guaranteedValue,
                                  SmallVectorImpl<Operand *> &usePoints,
                                  function_ref<bool(Operand *)> visitReborrow);

/// Find all "use points" of guaranteed value across its extended borrow
/// scope. The "use points" are the relevant points for determining lifetime.
///
/// A guaranteed value that introduces a local borrow scope (begin_borrow,
/// load_borrow, and phi) may have its lifetime "extended" via reborrows (other
/// guaranteed values are never reborrowed). Within this function,
/// findTransitiveGuaranteedUses() will be called iteratively at each reborrow,
/// after adding the reborrows as use points. The final use points may
/// no longer be dominated by the guaranteed value.
///
/// Accumulate results in \p usePoints. This avoids the need for a separate
/// worklist and result vector. Any existing elements are ignored.
///
/// Return true if all uses were analyzed and no PointerEscapes were
/// discovered. PointerEscapes are allowed, however, if they are within a borrow
/// scope either introduced by this guaranteed value or a nested borrow--these
/// escapes must still be contained within the scope, so they are irrelevant to
/// lifetime.
///
/// Note that if \p guaranteedValue is already known to introduce a local borrow
/// scope, then BorrowedValue::visitExtendedLocalScopeEndingUses() yields
/// equivalent results.
bool findExtendedTransitiveGuaranteedUses(
  SILValue guaranteedValue,
  SmallVectorImpl<Operand *> &usePoints);

/// An operand that forwards ownership of one operand to a result.
