...
static bool canFixUpOwnershipForRAUW(SILValue oldValue, SILValue newValue,
  ...
  // FIXME: the rest of the logic in this function a shim between OwnershipUtils
  // and OwnershipContextFixup that will be simplified in a follow-up commit.
  ...
  auto visitReborrow = [&](Operand *endScope) {
    ...
    // FIXME: also handle non-phi reborrows when they exist using a separate
    // SILValue list since we don't want to refer directly to phi values.
