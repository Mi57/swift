...
///
/// !!!FIXME: This does not handle borrows yet. It needs treat all the
/// !!!end-of-scope uses as uses too. To handle reborrows we also need to create
/// !!!copied. I have a separate utility to do that which isn't checked in yet.
bool swift::insertOSSADestroys(SILValue value) {
  ...
  for (auto *boundaryEdge : boundary.boundaryEdges) {
    ...
    insertedDestroy = true;
  }
  SmallPtrSet<SILInstruction*, 4> unclaimedConsumes;
  for (auto *use : value->getConsumingUses()) {
    unclaimedConsumes.insert(use->getUser());
  }
  for (SILInstruction *lastUser : boundary.lastUsers) {
    if (unclaimedConsumes.erase(lastUser))
      continue;

    SILBuilderWithScope::insertAfter(lastUser, [&](SILBuilder &b) {
      b.createDestroyValue(lastUser->getLoc(), value);
    });
    insertedDestroy = true;
  }
  assert(unclaimedConsumes.empty() && "consume does not end lifetime");
  return insertedDestroy;
}
