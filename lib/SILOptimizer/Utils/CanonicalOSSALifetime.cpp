...
#define DEBUG_TYPE "canonical-ossa-lifetime"
...
static bool computeCanonicalLiveness(CanonicalOSSALifetime &lifetime) {
  ...
  while (!defUseWorkList.empty()) {
    ...
    for (Operand *use : value->getUses()) {
      ...
      auto operandOwnership = use->getOperandOwnership();
      if (!operandOwnership) {
        continue; // ignore type-dependent operands which don't use the value
      }
      ...
      case OperandOwnership::DestroyingConsume:
        // Destroying a value does not force pruned liveness.
        lifetime.recordLifetimeEnd(use);
        break;
...
static void insertDestroyOnCFGEdge(SILBasicBlock *predBB, SILBasicBlock *succBB,
  ...
  auto *di =
      builder.createDestroyValue(succBB->begin()->getLoc(), lifetime.def());
...
static void insertDestroyAtInst(SILBasicBlock::iterator pos,
  ...
  auto *di = builder.createDestroyValue((*pos).getLoc(), lifetime.def());
  lifetime.consumes.recordFinalConsume(di);
...
static void findOrInsertDestroyInBlock(SILBasicBlock *bb, bool findDestroy,
                                       CanonicalOSSALifetime &lifetime) {
...
///
/// Observations:
/// - The lifetime.def() must be postdominated by some subset of its
...
static void findOrInsertDestroys(CanonicalOSSALifetime &lifetime) {
  ...
  for (auto *originalLifetimeEndBB : lifetime.getLifetimeEndBlocks()) {
    ...
    auto visitBB = [&](SILBasicBlock *bb, SILBasicBlock *succBB) {
      ...
      case PrunedLiveBlocks::LiveOut:
        // If succBB is not set, then we started traversal in the middle of the
        // live range, so don't need any edge destroys.
        if (succBB) {
          insertDestroyOnCFGEdge(bb, succBB, lifetime);
        }
        break;
      case PrunedLiveBlocks::LiveWithin: {
        // The liveness boundary is inside this block. Insert a final destroy
        // inside the block if it doesn't already have one.
        bool findDestroy = (succBB == nullptr);
        if (!lifetime.consumes.hasFinalConsume(bb)) {
          findOrInsertDestroyInBlock(bb, findDestroy, lifetime);
        }
        break;
      }
    ...
    // visiting each block.
    visitBB(originalLifetimeEndBB, nullptr);
