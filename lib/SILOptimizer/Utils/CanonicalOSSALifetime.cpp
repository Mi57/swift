...
static void insertDestroyOnCFGEdge(SILBasicBlock *predBB, SILBasicBlock *succBB,
  ...
  auto *di = builder.createDestroyValue(loc, def, poisonRefs);
...
void CanonicalizeOSSALifetime::findOrInsertDestroyInBlock(SILBasicBlock *bb) {
  ...
  while (true) {
    ...
    case PrunedLiveness::LifetimeEndingUse:
      ...
      //
      // In poisonRefsMode, the final consume must always be a destroy. There's
      // no way for a non-destroy to poison refs. And in the case of calls, we
      // can't poison the ref until after the call returns.
      if (!poisonRefsMode || isa<DestroyValueInst>(inst)) {
        consumes.recordFinalConsume(inst);
        return;
      }
      // If we can't use this instruction as a final consume, treat it like a
    ...
    case PrunedLiveness::NonLifetimeEndingUse:
      ...
      if (inst == bb->getTerminator()) {
        for (auto &succ : bb->getSuccessors()) {
          insertDestroyOnCFGEdge(bb, succ, currentDef, poisonRefsMode,
                                 consumes);
          setChanged();
        }
      } else {
        insertDestroyAtInst(std::next(instIter), existingDestroy, currentDef,
                            poisonRefsMode, consumes);
        setChanged();
      }
      return;
    }
    ...
    if (instIter == bb->begin()) {
      ...
      insertDestroyAtInst(instIter, existingDestroy, currentDef, poisonRefsMode,
                          consumes);
      setChanged();
      return;
    }
    ...
    case PrunedLiveBlocks::Dead:
      ...
      for (auto *predBB : bb->getPredecessorBlocks()) {
        if (liveness.getBlockLiveness(predBB) == PrunedLiveBlocks::LiveOut) {
          insertDestroyOnCFGEdge(predBB, bb, currentDef, poisonRefsMode,
                                 consumes);
          setChanged();
        } else {
          blockWorklist.insert(predBB);
