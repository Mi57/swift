...
//===----------------------------------------------------------------------===//
///
/// Note: A much simpler, cheaper alternative would be to simply delete all
/// borrow scopes unless they have a PointerEscape use, since destroy hoisting
/// will always respect non-PointerEscape uses. However, before making OSSA the
/// default, we will need to tag certain "semantically defined" borrow scopes
/// for 'self', function arguments, and scoped variables to prevent their scopes
/// from being removed while they still have inner uses. Optimizing those
/// defined scopes may end up requiring borrow scope canonicalization anyway.
///
