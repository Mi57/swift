...
//
// FIXME: The way DeadEndBlocks is used within this algorithm is illogical. It
...
bool ValueLifetimeAnalysis::computeFrontier(FrontierImpl &frontier, Mode mode,
  ...
  auto visitLastUser = [&](SILInstruction *lastUser) {
    ...
    // FIXME: This check makes no sense with respect to the DeadEndBlocks check
    // above, but I'm preserving existing behavior. Why would the presence of
    // dead-end blocks cause the analysis to fail?
    if (!deadInSucc) {
    ...
    // Bailout if the client expects a use point to be seen on all paths to the
    // function exit.
    //
    // FIXME: this check does not belong inside the liveness algorithm. The
    // client can check this invariant itself just by checking boundary edges.
    if (mode == UsersMustPostDomDef) {
      foundInvalidLastUser = true;
      return;
    }
    // For all successor blocks, their first instruction will be added to the
    // frontier. Fake this block as live-out so edge splitting works below.
  ...
  auto visitBoundaryEdge = [&](SILBasicBlock *predBB, SILBasicBlock *succBB) {
    // FIXME: It makes no sense to check for dead-end blocks on all
    // successors here, but I'm preserving the existing behavior. If the
    // idea is to avoid putting the dead-end block on the frontier, then why
    // not just check the current succBB?
    bool deadInSucc = false;
    for (const SILSuccessor &succ : predBB->getSuccessors()) {
      if (!deBlocks || !deBlocks->isDeadEnd(succ)) {
        deadInSucc = true;
      }
    }
