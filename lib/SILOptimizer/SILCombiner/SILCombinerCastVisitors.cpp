...
visitPointerToAddressInst(PointerToAddressInst *PTAI) {
  ...
  if (PTAI->isStrict()) {
    ...
    if (auto *ATPI = dyn_cast<AddressToPointerInst>(PTAI->getOperand())) {
      ...
      if (helper) {
        auto replacement = helper.getReplacementValue();
...
visitUncheckedBitwiseCastInst(UncheckedBitwiseCastInst *UBCI) {
...

  {
    ...
    if (helper) {
      auto replacement = helper.getReplacementValue();
      auto *newInst = Builder.createUncheckedRefCast(UBCI->getLoc(),
                                                     replacement,
                                                     UBCI->getType());
...
SILCombiner::visitConvertFunctionInst(ConvertFunctionInst *cfi) {
  ...
  if (cfi->onlyConvertsSubstitutions()) {
    ...
    while (!worklist.empty()) {
      ...
      // The code above performs the OSSA preparation normally handled within
      // OwnershipRAUWHelper. Update the helper's newValue so it uses the
      // cleaned up replacement. This assumes that any validity checks
      // OwnershipRAUWHelper's constructor remain valid for newConvert.
      //
      // TODO: Why is OwnershipRAUWHelper called here at all after manually
      // fixing OSSA?
      helper.newValue = newConvert;
      helper.perform();
    }
  }
