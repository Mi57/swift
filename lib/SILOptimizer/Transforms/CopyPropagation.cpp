  ...
  if (canonicalizeBorrows) {
    ...
    while (true) {
      while (!defWorklist.ownedForwards.empty()) {
        SILInstruction *ownedForward =
            defWorklist.ownedForwards.pop_back_val();
        ...
        if (sinkOwnedForward(ownedForward, postOrderAnalysis, domTree)) {
          ...
          // operand. This handle chained forwarding instructions that were
      ...
      if (borrowCanonicalizer.canonicalizeBorrowScope(borrow)) {
        //!!!
        auto *deBlocksAnalysis = getAnalysis<DeadEndBlocksAnalysis>();
        f->verifyOwnership(deBlocksAnalysis->get(f));
      }
