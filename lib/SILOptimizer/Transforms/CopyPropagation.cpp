...
///
/// [WIP] This is meant to complement -enable-sil-opaque-values. It is still an
/// incomplete proof of concept. Ownership properties should be exposed via an
/// API that makes this pass trivially sound.
///
/// State:
/// copiedDefs : {SILValue}
...
///
///    For each use:
///    - If the Use is consumed: add its block to destroyBlocks.
...
///    If a consumer != blockDestroys[consumer.parentBB]:
...
/// the same conditions as the more complicated ValueLifetimeAnalysis.
///
/// FIXME: SILGen currently emits spurious borrow scopes which blocks this
/// optimization.
///
/// TODO: Extend this algorithm to better handle aggregates. If ownership SIL
/// uses a `destructure` operation to split aggregates, then the current scalar
/// algorithm should work naturally. However, if we instead need to handle
/// borrow scopes, then this becomes much harder. We would need a DI-style
/// tracker to track the uses of a particular copied def. The single boolean
/// "live" state would be replaced with a bitvector that tracks each tuple
/// element.
///
