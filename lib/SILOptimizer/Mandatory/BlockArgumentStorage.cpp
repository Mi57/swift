...
// to reuse storage across block arguments as follows.
//
...
//   edge.  If any other source is live out of that predecessor, then this
//   predecessor block will be marked black when we process that other incoming
//   value.
//
...
// between arbitrary storage locations, which likely requires computing and
...
BlockArgumentStorageOptimizer::computeArgumentProjections() && {
  ...
  for (auto *incomingOper : incomingOperands) {
    ...
    // FIXME: !!! If the incoming def is a copy, transitively consider
    // coalescing the source of the copy by continuing the same liveness
    // traversal, treating the copy as a new use.
