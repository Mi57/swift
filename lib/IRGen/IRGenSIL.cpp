...
void IRGenSILFunction::emitPoisonDebugValueInst(DebugValueInst *i) {
  ...
  // If the current value uses any spare bits, then don't overwrite it with
  // poison. This way, lldb will correctly display the non-reference value
  // (e.g. Optional.None) rather than telling the user that an object was
  // deinitialized, when there was no object to begin with.
  if (!storageTy->isPointerTy()) {
    ...
    llvm::Value *onlySpareBits = Builder.CreateAnd(currentBits, mask);
    llvm::Value *zeroWord = llvm::ConstantInt::get(IGM.SizeTy, 0);
    llvm::Value *isPtr = Builder.CreateICmpEQ(onlySpareBits, zeroWord);
    poisonedVal = Builder.CreateSelect(isPtr, poisonedVal, currentBits);
  }
